...
export default async function SellerSalePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;

  // Middleware already verified auth + seller role and set x-user-id (#14)
  ...
  const { data: deal } = await supabase
    ...
    .select(`
      *,
      ...
      buyer:profiles!deals_buyer_id_fkey(full_name, email, phone, avatar_url, country),
  ...
  return <DealDetailView deal={deal} currentUserId={userId} role="seller" />;
}
